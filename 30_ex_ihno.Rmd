...
* `phobia`   indepedent variable (IC, X2): self-reported math phobia, score of 0-10
...
```{r}
df_ihno_use <- df_ihno %>% 
  dplyr::filter(complete.cases(statquiz, mathquiz, phobia))
```

```{r}
tibble::glimpse(df_ihno_use)
```
...
#### Unadjusted or Simple Regression
...
apaSupp::tab_lm(fit_ihno_lm_1,
                caption = "Parameter Estiamtes for Statistics Performance Regressed on Math Performance")
```
...
```{r}
apaSupp::tab_lm(fit_ihno_lm_2,
                caption = "Parameter Estiamtes for Statistics Performance Regressed on Math Phobia")
```
...
#### Adjusted or Multiple Regression
...
apaSupp::tab_lm(fit_ihno_lm_3,
                caption = "Parameter Estiamtes for Statistics Performance Regressed on Math Performance and Phobia")
```
...
```{r}
effectsize::r2_semipartial(fit_ihno_lm_3)
```

```{r}
effectsize::eta_squared(fit_ihno_lm_3, partial = TRUE)
```

```{r}
effectsize::eta_squared(fit_ihno_lm_3, partial = FALSE)
```
...
```{r}
apaSupp::tab_lms(list(fit_ihno_lm_1, fit_ihno_lm_2, fit_ihno_lm_3)) 
```
