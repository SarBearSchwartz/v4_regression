...
```{r}
apaTables::apa.aov.table(fit_ihno_lm_1,
                         conf.level = .95)$table_body %>% 
  data.frame() %>%  
  dplyr::filter(Predictor != "(Intercept)") %>% 
  dplyr::select(Predictor, SS, df, MS, F, p, "eta2_p" = partial_eta2) %>% 
  dplyr::mutate(p = apaSupp::p_num(as.numeric(p))) %>% 
  flextable::flextable() %>% 
  apaSupp::theme_apa(caption = "Regression Anova Table")
```
...
```{r}
apaTables::apa.aov.table(fit_ihno_lm_3,
                         conf.level = .95)$table_body %>% 
  data.frame() %>%  
  dplyr::filter(Predictor != "(Intercept)") %>% 
  dplyr::select(Predictor, SS, df, MS, F, p, "eta2_p" = partial_eta2) %>% 
