...
```{r, fig.cap="Distribution of Pre-test Score by Therapy Type"}
...
```{r, fig.cap="Distribution of Post-test Score by Therapy Type"}
apaSupp::spicy_histos(df = df_ptsd,
                      var = post_test,
                      split = therapy,
                      lab = "Post-Test Score") +
...
```{r, fig.cap="Distribution of Gain Score by Therapy Type"}
apaSupp::spicy_histos(df = df_ptsd,
                      var = gain,
                      split = therapy,
                      lab = "Gain on Test") +
...
```{r, fig.cap="Association Between Post-Test and Pre-Test, Stratified by Therapy"}
df_ptsd %>% 
  ...
  geom_smooth(aes(color = therapy,
              ...
              formula = y ~x,
              alpha = .2) +
  labs(x = "Observed Pre-Test Score",
...
df_ptsd %>% 
  ...
  geom_smooth(aes(color = therapy,
              ...
              alpha = .2) +
  labs(x = "Observed Pre-Test Score",
       y = "Observed GAIN, Post-Test vs. Pre-Test",
       color    = "Therapy:",
       fill     = "Therapy:",
       shape    = "Therapy:",
       linetype = "Therapy:") +
...
$$
\widehat{post} = 5.019 + 0.498(pre) -3.00(EXP)
$$
...
No evidene was found that the therapy is associated with a differential gain on the test,*b* = -3.00, *SE* = 1.80, *p* =.121.
...
After controlling for pre-test score, the experimental therapy is associated with 3 point lower gain on the post-test compared to the traditional therapy, *b* = -3.00, *SE* = 0.36, *p* <.001.
