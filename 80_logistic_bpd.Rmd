...
#### Deviance = -1 times the Log Likelihood
...
```{r}
AIC(fit_glm_0)  # Null Model
```


```{r}
AIC(fit_glm_1)  # Full Model
```
...
#### Likelihood Ratio Test (Deviance Difference) of all Nested Models
...
#### `texreg` exponentiate the betas (SE are not exp) with 95% confidence intervals
...
#### `texreg` with Logit and OR values BOTH
